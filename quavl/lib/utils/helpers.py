...
def swap_transform_non_adjacent_gate(gate: np.ndarray, gate_qbits: List[int], num_qbits: int) -> np.ndarray:
    ...
    gates = swap_gates + [identity_pad_gate(gate, [gate_qbits[1] - 1, gate_qbits[1]], num_gate_qbits)] + swap_gates[
                                                                                                         ::-1]
...
def build_qbit_constraints(qbit_identifiers: List[str], is_reduced_state_space: bool = False) -> str:
    """
    ...
    for identifier in qbit_identifiers:
        if is_reduced_state_space:
            if False:
                attributes = ['z0', 'z1', 'v0', 'v1'] # 'h0', 'h1', 'zm0', 'zm1', 'hm0', 'hm1',
            ...
            expr += f';Constraints for qbit {identifier}\n'
            expr += f';----------------------\n'
