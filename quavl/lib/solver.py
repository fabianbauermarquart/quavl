...
def write_smt_file(solver: Solver,
    ...
    if isinstance(qbits[0], str):
        ...
        smt_expr += build_qbit_constraints(qbits, is_reduced_state_space)
    elif isinstance(qbits[0], QbitVal):
        for qbit in qbits:
            qbit_identifiers.add(qbit)
        smt_expr += build_qbit_constraints(list(qbit_identifiers), is_reduced_state_space)
    ...
    if output_qbits is not None:
        smt_expr += build_qbit_constraints(output_qbits, is_reduced_state_space)
...
def _dict_group(dict: Dict,
    ...
    for qbit_key in all_qbit_keys:
        ...
        phi = np.mean(dict[f'{qbit_key}.phi'])
        theta = np.mean(dict[f'{qbit_key}.theta'])
